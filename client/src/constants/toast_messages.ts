// Biomechanical Reports
const TOAST_BIOMECH_GET: string = 'Unable to fetch biomech reports';
const TOAST_BIOMECH_POST: string = 'Unable to post biomech reports';

// Case studies
const TOAST_CASESTUDY_GET: string = 'Unable to fetch case study';
const TOAST_CASESTUDY_POST: string = 'Unable to add case study';
const TOAST_CASESTUDY_PATCH: string = 'Unable to set new featured case study';
const TOAST_CASESTUDY_DELETE: string = 'Unable to delete case study';

// Employee of the month
const TOAST_EMPLOYEE_OF_THE_MONTH_GET: string = 'Unable to fetch employee of the month';
const TOAST_EMPLOYEE_OF_THE_MONTH_PUT: string = 'Unable to update employee of the month';

// Image
const TOAST_IMAGE_BY_PATH: string = 'Unable to fetch image';

// Leaderboard
const TOAST_LEADERBOARD_GET: string = 'Unable to fetch leaderboard';

// Messageboard
const TOAST_MESSAGEBOARD_GET: string = 'Unable to fetch message';
const TOAST_MESSAGEBOARD_PUT: string = 'Unable to update message';

export {
  TOAST_BIOMECH_GET,
  TOAST_BIOMECH_POST,
  TOAST_CASESTUDY_GET,
  TOAST_CASESTUDY_POST,
  TOAST_CASESTUDY_PATCH,
  TOAST_CASESTUDY_DELETE,
  TOAST_EMPLOYEE_OF_THE_MONTH_GET,
  TOAST_EMPLOYEE_OF_THE_MONTH_PUT,
  TOAST_IMAGE_BY_PATH,
  TOAST_LEADERBOARD_GET,
  TOAST_MESSAGEBOARD_GET,
  TOAST_MESSAGEBOARD_PUT,
};
