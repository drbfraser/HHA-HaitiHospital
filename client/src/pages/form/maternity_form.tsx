...
function MaternityForm() {
  ...
  const onSubmit = async (data: any) => {
    ...
    if (submitValidation()) {
      ...
      for (var key in table) {
        ...
        table[key].Total = totalElement.innerText;
        index++;
      }
      ...
      for (var key in table['Total visits']) {
        ...
        table['Total visits'][key] = totalElement.innerText;
        index++;
      }
          ...
          <div className="row g-2 pb-2">
            <div className="col-sm-10 ps-5" id={'selectList' + fieldIndex}>
              ...
              <select
                ...
                onChange={(e) => selectList(e.target.value, state, field)}
              >
                  ...
                  <>
                    ...
                    <div id={'ListInputs' + fieldIndex + i + j} className="col-sm-2">
                      <input
                        ...
                        {...register(
                          item.field_parent + '.patient' + (i + 1) + '.' + item.field_id,
                        )}
  ...
  const sidePanelClick = (index: any) => {
    ...
    for (let i = 0; i < currentClass.length; i++) {
      ...
      for (let j = 1; j <= sections[i].section_fields.length; j++, startj++) {
        ...
        if (document.getElementById('list' + startj))
          document.getElementById('list' + startj)!.style.display = show;
      }
    }
  };
      ...
      if (field.field_type === 'array') {
        ...
        for (let idx = 0; idx < patientState; idx++) {
          for (let jdx = 0; jdx < field.field_template.length; jdx++) {
            inputElement = document.getElementById('ListInputs' + i + idx + jdx)
              ?.childNodes[0] as HTMLInputElement;
            isFormValid =
              listInputValidation(idx, jdx, field.field_template[jdx].field_type, i) && isFormValid;
          }
        }
      }
    }
      ...
      for (let j = 1; j <= section.section_fields.length; j++, num++) {
        ...
        if (field.field_type === 'array') {
          ...
          for (let k = 0; k < formValues.length; k++) {
            var textInput = inputGroup.childNodes[k].childNodes[0]
              .childNodes[0] as HTMLInputElement;
            var valueInput = inputGroup.childNodes[k].childNodes[1]
              .childNodes[0] as HTMLInputElement;
            if (
              textInput.classList.contains('is-invalid') ||
              valueInput.classList.contains('is-invalid')
            ) {
          ...
          if (document.getElementById('selectList' + num) !== null) {
            var selectList = document.getElementById('selectList' + num)
  ...
  function listInputValidation(i, j, type, fieldIndex) {
    var inputElement = document.getElementById('ListInputs' + fieldIndex + i + j)
      ?.childNodes[0] as HTMLInputElement;
    var selectList = document.getElementById('selectList' + fieldIndex)
      ?.childNodes[1] as HTMLInputElement;
    if (type === 'number') {
      if (!isValid(inputElement)) {
        makeValidity(
          selectList,
          false,
          i18n.t('departmentFormInputValidationOneOrMoreFieldsAreInvalid'),
        );
        return false;
      }
    } else if (type === 'text') {
  ...
  function totalValidation(start: number, a: number, b: number) {
    ...
    var isSeriesComplete =
      totalElement.classList.contains('is-valid') || totalElement.classList.contains('is-invalid');
    for (var i = a; i <= b; i++) {
      var inputElement = document.getElementById('inputs' + i)?.childNodes[0] as HTMLInputElement;
      isSeriesComplete =
    ...
    if (isSeriesValid) {
      if (total !== total2) {
        makeValidity(
          totalElement,
          false,
          i18n.t('departmentFormTotalValidationDoesNotAddUpToTotal'),
        );
        for (var i = a; i <= b; i++) {
  ...
  function tableInputValidation(num: number, idx: number, jdx: number) {
    ...
    if (num === 42) {
      for (var i = 0; i < 3; i++) {
        var rowTotal = 0;
        for (var j = 0; j < 5; j++) {
          inputElement = document.getElementById('tables' + num + i + j)
            ?.childNodes[0] as HTMLInputElement;
      ...
      for (var j = 0; j < 5; j++) {
        ...
        totalElement.innerText = String(colTotal);
      }

      var rowTotal = 0;
      for (var j = 0; j < 5; j++) {
        var element = document.getElementById('tables' + num + 3 + j);
        rowTotal += Number(element.innerText);
      }
          ...
          <div className="col-sm-12 col-md-10 col-lg-8 col-xl-7 col-xxl-6">
                  ...
                  ? sections.map((section: any, idx: any) => {
                      ...
                      for (let i = fieldCount + 1; i <= fieldCount + fields.length; i++) {
                        ...
                        if (field.subsection_label) {
                          ret.push(
                            <h6 id={'subsection' + i} className={'fw-bold' + indentClass}>
                                  ...
                                  <div className={'row g-2 mb-1 align-items-center' + indentClass}>
                                    ...
                                    <div className="col-sm-2 d-grid">
                                      <button
                                        ...
                                        onClick={() => removeFormFields(i, j)}
                                      >
                                        {t('departmentFormRemove')}
                                      </button>
                                    </div>
                                  </div>
                                ))}
            ...
            <div className="btn-group d-flex">
              ...
              <button
                className="w-100 btn btn-secondary btn-sm"
                onClick={clickNext}
                disabled={sectionState === 4 ? true : false}
              >
