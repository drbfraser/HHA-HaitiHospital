...
export const runNumericValidators: { [key: string]: Validator<number> } = {
  isEven: (x) =>
    x % 2 === 0
      ? true
      : { error: 'ODD_NUMBER', message: 'Please input an even number' },
  isOdd: (x) =>
    x % 2 !== 0
      ? true
      : { error: 'EVEN_NUMBER', message: 'Please input an odd number' },
  isPositive: (x) =>
    x >= 0
      ? true
      : { error: 'NEGATIVE_NUMBER', message: 'Please input a non-negative number' },
};
