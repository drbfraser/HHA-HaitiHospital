  ...
  .get(
    ...
    async (req: RequestWithUser, res: Response, next: NextFunction) => {
      try {
        ...
        const docs = await ReportCollection.find({ departmentId: deptId }).sort({
          reportMonth: 'desc',
        });
        const jsons = await Promise.all(docs.map((doc) => doc.toJson()));
...
// Fetch report by id
router
  .route(`/report/:${REPORT_ID_URL_SLUG}`)
...
router
  ...
  .put(requireJwtAuth, async (req: RequestWithUser, res: Response, next: NextFunction) => {
    try {
  ...
  .delete(
    ...
    async (req: RequestWithUser, res: Response, next: NextFunction) => {
      try {
        ...
        const substituteReport = await generateReportForMonth(
          doc.departmentId,
          doc.reportMonth!,
          req.user,
        );
        attemptToSaveReport(substituteReport, (err: CustomError) => {
          if (!err)
            return res
              ...
              .send(`Replace deleted report with an empty report`);
  ...
  .post(
    ...
    async (req: RequestWithUser, res: Response, next: NextFunction) => {
      try {
        ...
        const newReport = await generateReportForMonth(
          req.params[DEPARTMENT_ID_URL_SLUG],
          date,
          req.user,
        );
...
async function attemptToUpdateReport(
  report: ReportDescriptor,
  callback: (err?: CustomError) => void,
) {
